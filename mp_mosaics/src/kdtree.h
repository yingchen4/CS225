...
class KDTree
{
  ...
  private:
    ...
    void construct(vector<Point<Dim>>& newPoints, int start, int end, int dms,  KDTreeNode *& root);
    
    Point<Dim> find(KDTreeNode *subroot, const Point<Dim>& query, int dms) const ;

    typename KDTree<Dim>::KDTreeNode* copy(KDTreeNode* otherNode);
