...
class MM
{
    ...
    private:
        ...
        unsigned width_length_;
        unsigned height_length_;

        std::vector<std::vector<uint64_t>> minHashMatrix_;

        std::vector<uint64_t> computeMinHash(const cs225::PNG &input, unsigned tileX, unsigned tileY);
