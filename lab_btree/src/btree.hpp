...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
    if (!vector.empty() && first_larger_idx < vector.size()) {
        if (vector[first_larger_idx].key == key) {
            return vector[first_larger_idx].value;
        }
    }
    if (subroot->is_leaf) {
        return V();
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    // BTree: std::vector<DataPair> elements; std::vector<BTreeNode*> children;
    // DataPair: K key, V value;
    auto &vector = subroot->elements;
    size_t index = insertion_idx(vector, pair);
    if (subroot->is_leaf && (index == vector.size() || vector[index].key != pair.key)) {
        vector.insert(vector.begin() + index, pair);
    } else if (!subroot->is_leaf && (index == vector.size() || vector[index].key != pair.key)) {
